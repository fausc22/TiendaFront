...
const transporter = nodemailer.createTransport({
    ...
    auth: {
        user: 'csgosmurf251199@gmail.com',
        pass: 'Fausto.10'
    }
});
...
const sendEmail = (to, subject, data) => {
    const templatePath = path.resolve(__dirname, 'templates', 'order_confirmation.html');
    let htmlContent = fs.readFileSync(templatePath, 'utf8');

    // Reemplaza los marcadores en el HTML con los datos proporcionados
    for (let key in data) {
    ...
    const mailOptions = {
        from: 'csgosmurf251199@gmail.com',
        to: to,
        subject: subject,
        html: htmlContent
    };
...
app.post('/send-email', (req, res) => {
    const { to, subject, name, items, subtotal, shippingCost, total } = req.body;
    const itemsHtml = items.map(item => `
        <tr>
            <td align="left" style="font-family: Open Sans, Helvetica, Arial, sans-serif; font-size: 16px; font-weight: 400; line-height: 24px; padding: 10px;">
                ${item.name}
            </td>
            <td align="left" style="font-family: Open Sans, Helvetica, Arial, sans-serif; font-size: 16px; font-weight: 400; line-height: 24px; padding: 10px;">
                ${item.quantity}
    ...
    //FUNCION PARA LAS VARIABLES ENV
    app.get('/api/env', (req, res) => {
        ...
        console.log(response); // Verifica que todas las variables están siendo cargadas correctamente
        res.json(response);
      });
...
app.get('/api/variablesenv', (req, res) => {
    const config = {
      storeName: process.env.STORE_NAME,
      storeAddress: process.env.STORE_ADDRESS,
      storePhone: process.env.STORE_PHONE,
